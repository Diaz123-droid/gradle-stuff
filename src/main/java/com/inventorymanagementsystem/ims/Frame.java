...
public class Frame extends JFrame implements ActionListener {
    ...
    public DefaultMutableTreeNode LeftPaneTreeView() {
        ...
        DefaultMutableTreeNode rootNode = new DefaultMutableTreeNode("[IMS ROOT]");

        for (int index = 0; index < 7; index++) {
            DefaultMutableTreeNode node = new DefaultMutableTreeNode(String.format("[%s]", nodeTitles[index]));

            for (int i = 0; i < (int) (Math.random() * 10); i++) {
                node.add(new DefaultMutableTreeNode(MessageFormat.format("{0} Child {1}", nodeTitles[index], i)));
            }
